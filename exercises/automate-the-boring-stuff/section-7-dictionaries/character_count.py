...
"""
count = {}  # to find # of appearances for each character

# 1. tells count{} to start from 0 as character iterates over message
# 2. generates character : count

for character in message.upper():  # treats all characters as uppercase
    count.setdefault(character, 0)  # prevents KeyError
    count[character] = count[character] + 1

pprint.pprint(
    count
)  # pprint places the keys in order and provides a more readable format

# rjtext = pprint.pformat()
# print(rjtext) provides pprint as a string

"""
setdefault() creates a key, value pairing if none already exists. For each unique
character that appears as character iterates over message, key(character):0(value) is
created. e.g. when character iterates over I in message, line 8 creates I : 0.
line 9 places I : 0 in count{} and adds 1 each time the character appears. "I" appears
once, so the result is count{I : 1}. With .upper(), i/I appears 7 times.
"""
